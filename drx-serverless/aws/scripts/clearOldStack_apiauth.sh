...
echo "Clear Old Stack for API Authorizer"
...
for word in $(aws cloudformation list-stacks --stack-status-filter "CREATE_COMPLETE" "UPDATE_COMPLETE" "UPDATE_ROLLBACK_COMPLETE" "CREATE_FAILED" "DELETE_FAILED" "ROLLBACK_COMPLETE" |jq .StackSummaries[].StackName |grep "$stackName-$stage")
do
  if [ "\"$stage\"" == "$(aws cloudformation describe-stacks --stack-name ${word//\"} |jq .Stacks[0].Tags| jq 'from_entries' | jq .STAGE)" ]; then
      echo "${word//\"}"
      ...
      echo "old apiauth stack deleted."
  fi
done
