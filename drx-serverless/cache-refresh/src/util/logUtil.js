...
const LOG_EVENTS = require('../configuration/eventConstants.js');
const SENSITIVEDATALIST =
  require('../configuration/sensitiveDataList.json').map((x) => _.toUpper(x));
...
function handleLog(recordLevel = DEBUG_LEVEL, message = {}) {
  ...
  if (LOG_LEVEL_MAP.get(recordLevel) <= LOG_LEVEL_MAP.get(ALLOWEDLOGLEVEL)) {
    console.log(
      stringifyWithMask(hideSensitiveData, getSensitiveData(), logMessageBody)
    );
  }
}
