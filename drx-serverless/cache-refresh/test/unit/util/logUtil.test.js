...
const LOG_EVENTS = require('../../../src/configuration/eventConstants.js');
...
describe('logUtil test', () => {
  describe('#handleLog, test log level - set as DEBUG', () => {
    ...
    it('should log debug,error, info string when log debug, error, info, trace log.', () => {
      ...
      expect(console.log.args[1][0]).to.equal(
        JSON.stringify(TEST_LOG_STRING_ERROR)
      );
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_INFO))).to.be
        .true;
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_DEBUG))).to
        .be.true;
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_TRACE))).to
        .be.false;
    });
    afterEach(() => {
      sandbox.restore();
  ...
  describe('#handleLog, test log level - set as INFO', () => {
    ...
    it('should log debug,error, info string when log debug, error, info, trace log.', () => {
      ...
      expect(console.log.args[1][0]).to.equal(
        JSON.stringify(TEST_LOG_STRING_INFO)
      );
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_INFO))).to.be
        .true;
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_TRACE))).to
        .be.false;
      expect(console.log.neverCalledWith(JSON.stringify(TEST_LOG_STRING_DEBUG)))
        .to.be.true;
    });
    afterEach(() => {
      sandbox.restore();
  ...
  describe('#handleLog, test log level - set as INFO', () => {
    ...
    it('should log debug,error, info string when log debug, error, info, trace log.', () => {
      log.handleLog('INFO');
      expect(console.log.calledWith(JSON.stringify({}))).to.be.true;
    });
    afterEach(() => {
      sandbox.restore();
      delete process.env.logLevel;
  ...
  describe('#handleLog, test log level - no log level set', () => {
    ...
    it('should log debug,error, info string when log debug, error, info, trace log.', () => {
      ...
      log.handleLog('INFO', TEST_LOG_STRING_INFO);
      log.handleLog('TRACE', TEST_LOG_STRING_TRACE);
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_ERROR))).to
        .be.true;
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_INFO))).to
        .not.be.true;
      expect(console.log.calledWith(JSON.stringify(TEST_LOG_STRING_TRACE))).to
        .be.false;
      expect(console.log.neverCalledWith(JSON.stringify(TEST_LOG_STRING_DEBUG)))
  ...
  describe('#handleLog, test mask, should not mask', () => {
    ...
    it('should mask firstname only.', () => {
      ...
      expect(console.log.getCall(0).args[0]).to.include('testFirstName');
      expect(console.log.getCall(0).args[0]).to.include('firstRecipient');
      expect(console.log.getCall(0).args[0]).to.include(
        'secondRecipientFirstName'
      );
    });
  ...
  describe('#handleLog, test mask, should not mask', () => {
    ...
    it('should mask firstname only.', () => {
      ...
      expect(console.log.getCall(0).args[0]).to.include(30);
    });
    afterEach(() => {
      sandbox.restore();
      delete process.env.logLevel;
