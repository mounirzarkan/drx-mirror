...
async function createEnquiryWebCase(event, reqContext) {
  try {
    log.debug('=====> Enquiry: createEnquiryWebCase');
    ...
    log.debug('=====> Enquiry: createEnquiryWebCase: sanitisedBody = ' + sanitisedBody);

    const payloadJson = JSON.parse(sanitisedBody);

    const {EnquiryController} = controllers;
    const {EnvironmentConfiguration} = configurations;
    const configuration = new EnvironmentConfiguration(process.env);

    await configuration.init();

    const enquiryController = new EnquiryController(configuration);
    const data = await enquiryController.createEnquiryWebCase(payloadJson);

    return data;
    
  } catch (err) {
    return processEnquiryError(err);
  }
}

function processEnquiryError(err) {
  if (err) {
    log.error(err);
    if (err.response) {
  ...
  log.error('error caught in Enquiry function: ');
  console.error(err);
  log.error(err);
  let _statusCode = '500';
