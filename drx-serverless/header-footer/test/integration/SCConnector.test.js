...
describe('SUITE: SCConnector', () => {
  describe('GetHeaderResponse with isScHttpAgent = true', () => {
    ...
    before(async () => {
      ...
      resp = await scConnector.getHeaderResponse('us', 'en');
    });
    after(() => {
      sinon.restore();
    });
  ...
  describe('getFooterResponse with isScHttpAgent = true', () => {
    let scReqSpy, scConnector, resp;
    before(async () => {
      const config = {
        _scEndpoint: 'https://www2.cochlear.com/api/cochlear',
      ...
      scReqSpy = sinon.spy(scConnector.req, 'post');
      resp = await scConnector.getFooterResponse('us', 'en');
    });
    after(() => {
      sinon.restore();
    });
  ...
  describe('GetHeaderResponse with isScHttpAgent = false', () => {
    let scReqSpy, scConnector, resp;
    before(async () => {
      const config = {
        _scEndpoint: 'https://www2.cochlear.com/api/cochlear',
  ...
  describe('getFooterResponse with isScHttpAgent = false', () => {
    let scReqSpy, scConnector, resp;
    before(async () => {
      const config = {
        _scEndpoint: 'https://www2.cochlear.com/api/cochlear',
