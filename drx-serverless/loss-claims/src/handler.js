...
async function submitLossClaim(event, reqContext) {
  try {
    log.debug('=====> lossClaims: submitLossClaim');
    ...
    log.debug(
      '=====> loss-claims: submitLossClaim: sanitisedBody = ' + sanitisedBody
    );

    const payloadJson = JSON.parse(sanitisedBody);

    const {LossClaimsController} = controllers;
    const {EnvironmentConfiguration} = configurations;
    const configuration = new EnvironmentConfiguration(process.env);

    await configuration.init();

    const lossClaimsController = new LossClaimsController(configuration);
    const data = await lossClaimsController.submitLossClaim(payloadJson);
    ...
    return processLossClaimsError(err);
  }
}

function processLossClaimsError(err) {
  if (err) {
    log.error(err);
    if (err.response) {
  ...
  log.error('error caught in lossClaims function: ');
  console.error(err);
  log.error(err);
  let _statusCode = '500';
