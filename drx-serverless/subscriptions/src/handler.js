...
async function subscribeMicBatt(event, reqContext) {
  try {
    log.debug('=====> subscriptions: subscribeMicBatt');
    ...
    log.debug('=====> subscriptions: subscribeMicBatt: sanitisedBody = ' + sanitisedBody);

    const payloadJson = JSON.parse(sanitisedBody);

    const {SubscriptionsController} = controllers;
    const {EnvironmentConfiguration} = configurations;
    const configuration = new EnvironmentConfiguration(process.env);

    await configuration.init();

    const subscriptionsController = new SubscriptionsController(configuration);
    const data = await subscriptionsController.subscribeMicBatt(payloadJson);
    ...
    return processSubscriptionsError(err);
  }
}

function processSubscriptionsError(err) {
  if (err) {
    log.error(err);
    if (err.response) {
  ...
  log.error('error caught in subscriptions function: ');
  console.error(err);
  log.error(err);
  let _statusCode = '500';
