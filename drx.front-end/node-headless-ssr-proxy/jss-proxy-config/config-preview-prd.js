...
// Setup TLS to ignore SSL certificate error when calling to Cochlear sitecore
 ...
 */
let appName = process.env.SITECORE_JSS_APP_NAME || `drx`;
...
/**
 * function fixProxyRequestPath is to provide workaround for node-headless-proxy that been identified on https://github.com/Sitecore/jss/issues/140
 *  It will be used to remove the problem path that causing during encoding querystring handled by node-headless-proxy component
 */

const fixProxyRequestPath = (requestPath) => {
  // We only need to fix layout service requests, ignore anything else.
  if (requestPath.toLowerCase().indexOf(config.layoutServiceRoute.toLowerCase()) === -1) {
    return requestPath;
  }

  const parsedUrl = new URL(requestPath, config.apiHost);
...
const config = {
  ...
  serverBundle,
  /**
   * siteNameTemplateRoute: replace ${siteName} with defined value.
   */
  siteNameTemplateRoute: "${siteName}-preview",
  /**
   ...
   */
  apiHost: process.env.SITECORE_API_HOST || 'https://cd.prod.cms.cochlear.cloud',
   ...
   * Required.
   */
  apiKey: process.env.SITECORE_API_KEY || serverBundle.apiKey,
  /**
  ...
  proxyOptions: {
    ...
    onProxyReq: (proxyReq, req, res) => {
      ...
      // added by Tony to provide workaround for node-headless-proxy issue described previously"
