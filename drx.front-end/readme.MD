...
## running app in mock mode

`REACT_APP_TEST_VAR=LOCAL jss start:connected`

## Disconnected Sitecore-less Development

Use `jss start` to run a local development server. Sitecore is not required to be installed, and content data is pulled 
from static local sources.

Disconnected development is appropriate when no Sitecore instance is available, or when frontend developers do not wish 
to run a local copy of Sitecore.
...
- Review the application config patch file(s) in `/sitecore/config` to ensure that it is configured appropriately for 
  your Sitecore installation.
- `jss deploy config` to deploy the Sitecore config patch files to the Sitecore instance (you may need to add the 
...
## if you want to change port number to 8080 - sometimes required for testing when using browserstack for example

\$ export PORT=8080

## to run in disconnected mode,
...
Add drx variables into the "jss-build-parameters" folder and update each environment file.

Update the ./scripts/generate-scripts.js file to include a reference to the new variable.

When the project is run, a temp config file is generated which will contain the output of all the environment variables. This file is imported into the project config file.

Add a reference to the new variable in the ./src/config.js file and this will make it available to the app with an import statement.

All local environment variables can be added to the root ./scjssconfig.json file.

Default: Set to SIT environment.
