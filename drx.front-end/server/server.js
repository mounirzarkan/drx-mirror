...
// required here as we are not using GraphQLClientFactory
...
import siteResolver from '../src/SiteResolver.js';
...
export function parseRouteUrl(url) {
  ...
  let result = null;
  const { lang, siteName } = siteResolver(url);
  ...
  if (result.country && result.lang) {
    const siteNameTemplateRoute = config.siteNameTemplateRoute || '${siteName}';
    const siteNameRoute = siteNameTemplateRoute.replace('${siteName}',siteName);
    result.siteName = siteNameRoute
    result.qsParams = `sc_site=${siteNameRoute}`;
  }
  return result;
}
...
function initializei18n(state) {
  ...
  return i18ninit(
    state.sitecore.context.language,
    state.viewBag.dictionary,
  );
}
