import React, { useState } from 'react';
import {
  Placeholder,
  VisitorIdentification,
} from '@sitecore-jss/sitecore-jss-react';
import { withTranslation } from 'react-i18next';
import Helmet from 'react-helmet';

import UniversalHeader from './components/UniversalHeader/UniversalHeader';

// import '@cochlear-design-system/foundation/dist/index.css';
import './styles/main.scss';
import './assets/app.scss';

/*
  APP LAYOUT
  This is where the app's HTML structure and root placeholders should be defined.

  All routes share this root layout by default (this could be customized in RouteHandler),
  but components added to inner placeholders are route-specific.
*/

// This is boilerplate navigation for sample purposes. Most apps should throw this away and use their own navigation implementation.
// Most apps may also wish to use GraphQL for their navigation construction; this sample does not simply to support disconnected mode.

let Navigation = ({
  token,
  tokenDetails,
  attributeDetails,
  accessToken,
  routeParams,
  url,
  getHeaderFooterDetails,
}) => {
  return (
    <UniversalHeader
      token={token}
      tokenDetails={tokenDetails}
      attributeDetails={attributeDetails}
      accessToken={accessToken}
      routeParams={routeParams}
      url={url}
      getHeaderFooterDetails={getHeaderFooterDetails}
    />
  );
};

// inject dictionary props (`t`) into navigation so we can translate it
// NOTE: using this is needed instead of using i18next directly to keep
// the component state updated when i18n state (e.g. current language) changes
Navigation = withTranslation()(Navigation);

const Layout = ({
  route,
  token,
  tokenDetails,
  attributeDetails,
  accessToken,
  routeParams,
  url,
  labels,
}) => {
  const [showGivenNameLast, setShowGivenNameLast] = useState(false);

  const getHeaderFooterDetails = ({ givenNameLast }) => {
    setShowGivenNameLast(givenNameLast);
  };

  return (
    <React.Fragment>
      {/* react-helmet enables setting <head> contents, like title and OG meta tags */}
      <Helmet>
        <title>
          {route.fields?.Title?.value ||
            'Cochlear | Hear Now And Always | Cochlear Implants'}
        </title>
      </Helmet>

      {/*
      VisitorIdentification is necessary for Sitecore Analytics to determine if the visitor is a robot.
      If Sitecore XP (with xConnect/xDB) is used, this is required or else analytics will not be collected for the JSS app.
      For XM (CMS-only) apps, this should be removed.

      VI detection only runs once for a given analytics ID, so this is not a recurring operation once cookies are established.
    */}
      <VisitorIdentification />
      <a
        id="skiplink"
        className="is-sr-only-focusable"
        href="#skipToContent"
      >
        <div className="container">
          <span className="skiplink-text">Skip to main content</span>
        </div>
      </a>

      <Navigation
        token={token}
        tokenDetails={tokenDetails}
        attributeDetails={attributeDetails}
        accessToken={accessToken}
        routeParams={routeParams}
        url={url}
        getHeaderFooterDetails={getHeaderFooterDetails}
      />

      {/* root placeholder for the app, which we add components to using route data */}
      <main className="site__content">
        <div id="skipToContent" tabIndex="-1" />
        <div className="content__wrapper">
          {token && (
            <Placeholder
              name="drx-main"
              rendering={route}
              token={token}
              tokenDetails={tokenDetails}
              attributeDetails={attributeDetails}
              routeParams={routeParams}
              labels={labels}
              url={url}
              headerFooterDetails={{ showGivenNameLast }}
            />
          )}
        </div>
      </main>
    </React.Fragment>
  );
};

export default Layout;
